...
const Medications = ({route,navigation}) => {
  const {pid}=route.params;
 // var pid='345'
// console.log('hi')
 const [aller,setAller]=useState();
 const allRef=firebase.firestore().collection('MEDICATION')
var allergy=[]
useEffect(()=>{
  //console.log('meow')
allRef.onSnapshot(
        ...
        <Pressable>
          <View style={styles.allergy_list}>
            <View style={styles.al_main}>
                <Text style={[styles.allergy_list_contents,{fontWeight:'bold'}]}>Medicine: {item.title}</Text>
                <TouchableOpacity style={{left:30}} onPress={()=>console.log("wants to edit")}>

                <FontAwesome5 name ="pen" size={30} color={'rgb(245,0,0)'}/>
                </TouchableOpacity>
            </View>
               <Text style={[styles.al_submain,{fontWeight:'bold'}]}>Illness: {item.cause}</Text>
            </View>
            <View>
              <Text>{'\n'}</Text>
            </View>
...
/>
          ...
          <Text style={styles.al_submain}>Tingling in mouth</Text>
            ...
            */}
           <TouchableOpacity onPress={()=>navigation.navigate('AddMed',{pid:pid})}>
